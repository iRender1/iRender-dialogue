...
RegisterNetEvent("print", function(text)
...
RegisterNetEvent("menu-iRender:showMenu", function(npc)
    SendNUIMessage({
        type = "dialog",
        options = npc.options,
        name = npc.name,
...
RegisterNUICallback("menu-iRender:hideMenu", function()
    SetNuiFocus(false, false)
    DestroyCamera()
end)

RegisterNUICallback("menu-iRender:process", function(data)
    ...
    while true do
        ...
        for _, npc in ipairs(Config.npcs) do
            ...
            if distance < 1.5 then
                DisplayHelpText("Interact by pressing the E key")
                if IsControlJustPressed(0, 38) then -- E tuşuna basıldığında
                    TriggerEvent("menu-iRender:showMenu", npc)
